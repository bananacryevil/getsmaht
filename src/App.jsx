...
export default function App() {
  ...
  const startRandomExercise = useCallback(() => {
    const candidates = items.filter(it => !it.completed && (it.exercises?.length || 0) > 0);
    if (!candidates.length) return;
    const choice = candidates[Math.floor(Math.random() * candidates.length)];
    openExerciseModal(choice.day, choice.title, choice.exercises);
  }, [items]);
          ...
          <aside className="space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
              <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-5 py-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M13 7H7v6h6V7z"/></svg>
                  Today’s Plan
                </h2>
              </div>
                ...
                {nextIncomplete ? (
                  <>
                    ...
                    <button
                      ...
                      className="w-full px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg font-semibold transition-colors"
                    >
                      Jump to Day →
                    </button>
                  </>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-5">
              <h3 className="font-bold text-slate-900 mb-3 flex items-center gap-2">
                <svg className="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4"/></svg>
                Upcoming Deliverables
              </h3>
              {deliverables.length ? (
                <ul className="space-y-2">
                  {deliverables.map(d => (
                    <li key={d.day} className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
                      <div>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-xs bg-slate-900 hover:bg-slate-800 text-white rounded-md font-semibold"
                      >
                        Jump
                      </button>
                    </li>
                ...
                <div className="text-sm text-slate-600">No pending deliverables.</div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {lastDone.day && (
                  <button
                    onClick={() => scrollToDay(lastDone.day)}
                    className="px-3 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-sm font-semibold"
                  >
                    Last Completed
                  </button>
                )}
